...
const storage = multer.diskStorage({
  async destination(req, _file, cb) {
    ...
    try { await ensureDir(dest); } catch {}
    cb(null, dest);
  },
...
export async function listStopImages(req, res) {
  try {
    ...
    const data = await listStopImagesByStop(req.params.stopId, { page, limit });
...
export async function deleteStopImageById(req, res) {
  try {
    ...
    // remove DB row
    const ok = await deleteStopImage(stopId, imageId, req.user.username, req.user.role === 'admin');
...
export async function listReviews(req, res) {
  try {
    ...
    const data = await listStopReviews(req.params.stopId, { page, limit, sort });
    return res.json(data);
  } catch (e) {
