DROP VIEW stop_events_today;
CREATE VIEW stop_events_today AS
SELECT
  st.route_id,
  r.route_short_name,
  r.route_color,
  ...
  s.stop_code,
  s.stop_name,
  st.arrival_time,                      -- scheduled
  st.departure_time,                    -- scheduled
  st.stop_sequence,
  u.arrival_delay,
  u.departure_delay,
  CASE WHEN u.trip_id IS NOT NULL THEN 1 ELSE 0 END AS real_time_data,
  /* seconds with delay applied */
  ...
  /* formatted estimated times (HH could be >=24 if your GTFS crosses midnight) */
  printf('%02d:%02d:%02d',
